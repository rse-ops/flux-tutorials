...
function draw_pie_languages(areaID, width, height) {

    width = width || stdTotalWidth
    height = height || stdTotalHeight

    var url = '{{ site.baseurl }}/api//languages.json';
    var files = [url];
    Promise.all(files.map(url => d3.json(url))).then(response => {
            var data = reformatData({"data": response[0]});
            drawGraph(data, areaID, width, height);
        });

    // Draw graph from data
    function drawGraph(data, areaID, width, height) {
        var graphHeader = 'RADIUSS Languages';
