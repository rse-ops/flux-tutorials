...
function draw_sunburst_licenses(areaID, width, height) {

    width = width || stdTotalWidth
    height = height || stdTotalHeight

    var url = '{{ site.baseurl }}/api/repos.json';
    var files = [url];
    Promise.all(files.map(url => d3.json(url))).then(values => drawSunburst({"data": values[0]}, areaID, width, height));

    function drawSunburst(obj, areaID, setwidth, setheight) {
        let data = reformatData(obj, null);

        const graphHeader = 'RADIUSS Licenses';

        const margin = { top: stdMargin.top, right: stdMargin.right, bottom: stdMargin.bottom / 2, left: stdMargin.left },
            width = setwidth * 2 - margin.left - margin.right,
            height = setheight * 2 - margin.top - margin.bottom;
